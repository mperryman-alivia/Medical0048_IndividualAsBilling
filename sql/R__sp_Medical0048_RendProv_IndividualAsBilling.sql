CREATE OR ALTER PROCEDURE [dbo].[sp_Medical0048_RendProv_IndividualAsBilling] @querystring nvarchar(max)

AS
BEGIN

-- 730048656    WENDY MARIE WRIGHT-BELL    730086886    ANNA  HASTINGS
DROP TABLE IF EXISTS #Medical0048_RendProv_TempMedClaim
CREATE TABLE #Medical0048_RendProv_TempMedClaim (
...
SET @Query  = CONCAT(
'
INSERT INTO #Medical0048_RendProv_TempMedClaim
SELECT [ClaimSeq]
...
DECLARE @ExecutionID varchar(255) = (SELECT TOP 1  at_ai_sched_proc_exec_id from #Medical0048_RendProv_TempMedClaim)
...
BEGIN
	...
	CREATE CLUSTERED INDEX IX_ClaimSeq ON #Medical0048_RendProv_TempMedClaim (ClaimSeq, sec_organization_id, sec_tenant_id)
	...
	FROM #Medical0048_RendProv_TempMedClaim ar 
...
INSERT INTO dbo.[Medical0048_IndividualAsBilling_RendProvMetrics]
SELECT cl.RenderingProviderAnalyticsID
	,MAX(RenderingProviderName) RenderingProviderName
	,MAX(RenderingProviderAnalyticsPeerGroup) RenderingProviderAnalyticsPeerGroup
...
AND cl.RenderingProviderAnalyticsID <> 'ERROR'

GROUP BY cl.RenderingProviderAnalyticsID, cl.sec_organization_id, cl.sec_tenant_id
